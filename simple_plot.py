...
plt.figure()
plt.plot(range(len(_timestamp)), accepted_conn, linewidth=0.1)
#plt.xticks(range(len(_timestamp)), _timestamp, size=2, rotation=90)
plt.savefig("accepted_conn.png", dpi=1000)

plt.figure()
plt.plot(range(len(_timestamp)), listen_queue, linewidth=0.1)
#plt.xticks(range(len(_timestamp)), _timestamp, size=2, rotation=90)
plt.savefig("listen_queue.png", dpi=1000)

"""
plt.figure()
plt.plot(range(len(_timestamp)), active_processes, linewidth=0.1)
plt.plot(range(len(_timestamp)), total_processes, linewidth=0.1)
plt.xticks(range(len(_timestamp)), _timestamp, size=2, rotation=90)
plt.savefig("processes.png", dpi=1000)
"""

plt.figure()
plt.plot(range(len(_timestamp)), _elapsed, linewidth=0.1)
#plt.xticks(range(len(_timestamp)), _timestamp, size=2, rotation=90)
